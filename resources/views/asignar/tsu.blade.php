...
@section('content_header')
   <h1>Insertar Alumno de TSU</h1>
@stop

@section('content')
<form action="/tsuasignar" method="POST">
  @csrf
  <div class="mb-3">
  ...
  <select class="form-control" required aria-label="Default select example" name="docente">
    <option selected value="">Docente</option>
    @foreach ($docentes as $docente )
        <option value="{{$docente->id}}">{{$docente->name}} {{$docente->paterno}} {{$docente->materno}}</option>
